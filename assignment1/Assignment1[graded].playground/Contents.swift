...
func busyStudents(arrayOfSets:[Set<String>])->Set<String>{
    ...
    for var index=0;index<arrayOfSets.count;++index{
        ...
        {
            commonStudents.intersectInPlace(arrayOfSets[index]) //each loop replaces commonStudent varible              with common elemets of two compared sets
        }
...
busyStudents([courseA,courseB])

busyStudents([courseA,courseB,courseC,courseD])
...
func cost(dictionary:[String:String])->Double{
    ...
    if  let priceValue=dictionary["price"], quantityValue=dictionary["quantity"] //to check for nil value and access optionals if not nill
...
func wordCount(words sampleString:String,count:Int)->[String:Int]{
     ...
     let wordCollection=sampleString.componentsSeparatedByCharactersInSet(NSCharacterSet.whitespaceCharacterSet()) //extract array  of words in a string separated by space
...
func wordCount2(words sampleString:String,count:Int = 2)->[String:Int]{ //takes default int value=2
    ...
    let wordCollection=sampleString.componentsSeparatedByCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
...
func wordCount3(number:Int)->((words :String)->[String:Int]){
    
    
    func countAgain(words stringSample:String)->[String:Int]{
        
    let wordCollection=stringSample.componentsSeparatedByCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
...
testA(words: "cat bat cat rat mouse bat")

testA(words: "a a a b c c")

let testB = wordCount3(3)

testB(words: "a a a b c c")
...
func evaluate(number:Int,another:(Int)->Double)->[Double]{
    ...
    for number in 1...number{
        ...
        returnArray.append(polyTable2(number))
...
evaluate(3,another:polyTable2)

evaluate(3,another:{Double(($0 ** 3) + (2 * $0) + 4)})

evaluate(10,another:{Double(($0 ** 3) + (2 * $0) + 4)})//polytable can be replaced by a closure function
...
func cost2(dictionary:[String:Any])->Double{
    ...
    if  let priceValue=dictionary["price"] as? String , quantityValue=dictionary["quantity"] as? String
    ...
    else if let priceValue=dictionary["price"] as? Double , quantityValue=dictionary["quantity"] as? Int{
    ...
    else if let priceValue=dictionary["price"] as? String , quantityValue=dictionary["quantity"] as? Int{
    ...
    else if let priceValue=dictionary["price"] as? Double , quantityValue=dictionary["quantity"] as? String{
...
cost2(["name":"Mochie Green Tea", "quantity": "2", "price": "2.3"])
...
cost2(["name":"Mochie Green Tea", "price": "2.3"])


cost2(["name":"Mochie Green Tea", "quantity": "2"])

cost2(["name":"Mochie Green Tea"])
...
//: 2 polyTable 10/10
//:
//: 3 busyStudents 10/10
//:
//: 4 average[Ints]->Double? 10/10
//:
//: 5 average2[Ints?]->Double? 10/10
//:
//: 6 cost 10/10
//:
//: 7 wordCount10/10
//:
//: 8 wordCount2 default 10/10
//:
//: 9 wordCount3 function 0/10
//:
//: 10 poly function 10/10
//:
//: 11dictionary - Multiple types 5/5
//:
//:  total  105/100
